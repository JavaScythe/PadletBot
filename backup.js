...
async function evalulate(raw, author, displayname, postH, num){
  ...
  if(raw == 'ping'){
    ...
    return str;
  } else if(raw[0] == 's' && raw[1] == 'e' && raw[2] == 'a' && raw[3] == 'r' && raw[4] == 'c' && raw[5] == 'h' && raw[6] == ' '){
    var string = '';
    for(s=7;s<raw.length;s++){
      string = string+raw[s];
    }
    var localurl = 'https://api.dictionaryapi.dev/api/v2/entries/en_US/'+string;
...
try {
  ...
  while(true){//null handle
    ...
    if (compare != count2 && post.children[count].children[0].children[0].children[2] != null){
      var postcont = post.children[count].children[0].children[0].children[2].children[0].children[0].children[0].textContent;
      var author = post.children[count].children[0].children[0].children[1].children[0].getAttribute('href');
      var displayname = post.children[count].children[0].children[0].children[1].children[0].children[1].textContent;
      var string = '';
